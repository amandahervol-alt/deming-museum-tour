...
# Title fonts: White lettering with dark drop shadow (just like Transportation Annex)
...
# Dark drop shadow (black/charcoal with offset)
shadow_color = (25, 18, 15, 230)
white_color = (255, 255, 255, 255)

# 2px down-right drop shadow
...
# Generate QR code for Room 5 (Bataan Death March Display)
cx = 382
cy = 655
r = 86
qr_size = 120
...
# White round medallion with fine terracotta rim (exactly like user's reference image)
...
artifact_jpg = os.path.join(artifact_dir, "bataan_death_march_white_text_poster.jpg")
...
if os.path.exists(backup_dir):
    rgb_result.save(os.path.join(backup_dir, "bataan_death_march_poster_placard.jpg"), quality=98)

# Test OpenCV scannability
detector = cv2.QRCodeDetector()
mat = cv2.imread(out_jpg)
val, pts, _ = detector.detectAndDecode(mat)
print(f"Bataan White Lettering Poster: Scannable? -> {bool(val)} | Decoded: '{val}'")
