...
# 3. White Lettering with dark drop shadow (Matching Transportation Annex & Medical Room)
...
# 4. Generate Room 10 QR code in center between birds
cx = 382
cy = 655
r = 86
qr_size = 120
...
# 5. Save poster files
out_jpg = "qr_codes/quilt_and_lace_room_poster_placard.jpg"
out_png = "qr_codes/quilt_and_lace_room_poster_placard.png"
rgb_result = result.convert("RGB")
rgb_result.save(out_jpg, quality=98)
result.save(out_png)

# Also save alias
rgb_result.save("qr_codes/quilt_lace_room_poster_placard.jpg", quality=98)

# Save artifact
artifact_dir = r"C:\Users\manet\.gemini\antigravity-ide\brain\6f34c999-2401-441a-aae3-0a0320e6d5be"
artifact_jpg = os.path.join(artifact_dir, "quilt_and_lace_room_poster_placard.jpg")
...
if os.path.exists(backup_dir):
    rgb_result.save(os.path.join(backup_dir, "quilt_and_lace_room_poster_placard.jpg"), quality=98)

# Verify OpenCV scannability
detector = cv2.QRCodeDetector()
mat = cv2.imread(out_jpg)
val, pts, _ = detector.detectAndDecode(mat)
print(f"Quilt & Lace Room Poster: Scannable? -> {bool(val)} | Decoded: '{val}'")
