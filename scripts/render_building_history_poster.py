...
# 1. Update Room 3 in rooms.json
with open("data/rooms.json", "r", encoding="utf-8") as f:
    rooms = json.load(f)

for r in rooms:
    if r["id"] == 3:
        r["title"] = "Building History"
        r["subtitle"] = "Historic 1916 National Guard Armory & Architecture"
        print("Updated Room 3 title to 'Building History' in rooms.json")
...
knowledge["key_collections"]["building_history"] = {
    "building": "Historic 1916 Red-Brick National Guard Armory listed on the National Register of Historic Places.",
    "military_origin": "Built for New Mexico National Guard Company I during intense Southwestern border tensions.",
    "architecture": "Soaring ceilings, exposed heavy timber roof trusses, and original vintage brickwork.",
    "preservation": "Converted by Luna County Historical Society into community center and museum."
}

with open("data/museum_knowledge.json", "w", encoding="utf-8") as f:
    json.dump(knowledge, f, indent=2, ensure_ascii=False)

# 2. Configuration for Building History (Room 3)
ROOM_ID = 3
...
# 3. White Lettering with dark drop shadow (Matching all museum placards)
...
# 4. Generate Room 3 QR code in center between birds (cx=382, cy=655)
cx = 382
cy = 655
r = 86
qr_size = 120
...
# 5. Save poster files
out_jpg = "qr_codes/building_history_poster_placard.jpg"
out_png = "qr_codes/building_history_poster_placard.png"
rgb_result = result.convert("RGB")
rgb_result.save(out_jpg, quality=98)
result.save(out_png)

# Also save main_room alias
rgb_result.save("qr_codes/main_room_poster_placard.jpg", quality=98)
result.save("qr_codes/main_room_poster_placard.png")

# Save artifact
artifact_dir = r"C:\Users\manet\.gemini\antigravity-ide\brain\6f34c999-2401-441a-aae3-0a0320e6d5be"
artifact_jpg = os.path.join(artifact_dir, "building_history_poster_placard.jpg")
...
if os.path.exists(backup_dir):
    rgb_result.save(os.path.join(backup_dir, "building_history_poster_placard.jpg"), quality=98)
    rgb_result.save(os.path.join(backup_dir, "main_room_poster_placard.jpg"), quality=98)

# Verify OpenCV scannability
detector = cv2.QRCodeDetector()
mat = cv2.imread(out_jpg)
val, pts, _ = detector.detectAndDecode(mat)
print(f"Building History Poster: Scannable? -> {bool(val)} | Decoded: '{val}'")
